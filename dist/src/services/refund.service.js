...
export class RefundService {
    static async createRefund(data, cashierId) {
        const order = await prisma.order.findUnique({
            where: { id: data.orderId },
            include: { branch: true }
        });
        if (!order)
            throw new ResourceNotFoundError("Order", data.orderId);
        ...
        if (!cashier)
            throw new ResourceNotFoundError("User", cashierId);
    ...
    static async getRefundsByBranch(branchId) {
    ...
    static async getRefundsByCashier(cashierId) {
        return await prisma.refund.findMany({
            where: { cashierId },
            include: {
                order: {
