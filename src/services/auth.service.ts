...
export class AuthService {            
  ...
  static async verifyResetToken(data: VerifyResetTokenInput) {
    const resetToken = await prisma.passwordResetToken.findFirst({
      where: { token: data.token },
    });
  ...
  static async sendEmailOTP(data: EmailOtpInput) {
    const email = data.email.trim().toLowerCase();
    ...
    console.log(
      `[AuthService.sendEmailOTP] existing otp record: ${existingOtp ? "found" : "not found"}`
    );
    ...
    if (updated.count === 0) {
      try {
        ...
        if (this.isUniqueConstraintError(error)) {
          ...
          console.log(
            `[AuthService.sendEmailOTP] concurrent create detected, otp record updated for ${safeEmail}`
          );
        } else {
          throw error;
        }
  ...
  static async verifyEmailOTP(data: VerifyEmailOtpInput) {
    const email = data.email.trim().toLowerCase();
