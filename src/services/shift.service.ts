...
export class ShiftReportService {
  static async startShift(cashierId: number, openingBalance: number) {
  ...
  static async getCurrentShift(cashierId: number) {
    ...
    const productSales = new Map<number, { id: number, name: string, sellingPrice: number, quantity: number }>();
  ...
  static async endShift(shiftId?: number, cashierId?: number) {
    let shift;
    if (shiftId) {
    ...
    orders.forEach(order => {
      totalSales += order.finalAmount;
      if (order.paymentType === PaymentType.CASH) cashSales += order.finalAmount;
      else if (order.paymentType === PaymentType.CARD) cardSales += order.finalAmount;
      else if (order.paymentType === PaymentType.UPI) upiSales += order.finalAmount;
    });
  ...
  static async getShiftsByBranch(branchId: number) {
  ...
  static async getShiftReportByDate(cashierId: number, date: Date) {
  ...
  static async getShiftById(id: number) {
    ...
    if (!shift) throw new ResourceNotFoundError("ShiftReport", id);
    return shift;
  }

  static async getCashierShifts(cashierId: number) {
    return await prisma.shiftReport.findMany({
      where: { cashierId },
      orderBy: { startTime: 'desc' }
    });
