...
export class ProductService {
  static async createProduct(data: CreateProductInput) {
    ...
    if (existingProduct) throw new UserException(`Product with SKU ${data.sku} already exists`);
  ...
  static async bulkImportProducts(storeId: number, buffer: Buffer) {
  ...
  static async getProductsByStore(storeId: number, search?: string) {
  ...
  static async updateProduct(id: number, data: UpdateProductInput) {
    ...
    if (data.sku && data.sku !== product.sku) {
      ...
      if (existing) throw new UserException("New SKU is already taken");
    }
